...
namespace CSnakesTransformersMemoryLeak.Python.TransformersSharp.Tokenizers
{
	/// <summary>
	...
	public class PreTrainedTokenizerBase : Tokenizer
	{
		...
		/// </summary>
		protected readonly PyObject TokenizerInstance;

		private bool AddSpecialTokens;

		public readonly bool IsFastTokenizer;
		...
		/// </summary>
		/// <param name="tokenizerInstance">The Python tokenizer object to wrap.</param>
		/// <param name="addSpecialTokens">Indicates whether to add special tokens during tokenization. Default is <c>true</c>.</param>
		internal PreTrainedTokenizerBase(PyObject tokenizerInstance, bool addSpecialTokens = true)
		{
		...
		public PreTrainedTokenizerBase(
			...
			bool useFastTokenizer = true)
		{
			var transformersWrapper = TransformersWrapper = transformersEnvironment.TransformersWrapper;

			var tokenizerInstance = TokenizerInstance = transformersWrapper.TokenizerFromPretrained(
				model: model,
				cacheDir: cacheDir,
				forceDownload: forceDownload,
				revision: revision,
				trustRemoteCode: trustRemoteCode,
				useFast: useFastTokenizer
			);

			AddSpecialTokens = addSpecialTokens;

			IsFastTokenizer = tokenizerInstance
				.GetAttr("is_fast")
				.As<bool>();
		}
		...
		{
			return TransformersWrapper.TokenizerTextAsNdarray(TokenizerInstance, text).AsInt64ReadOnlySpan();
		}
		...
		protected override EncodeResults<EncodedToken> EncodeToTokens(string? text, ReadOnlySpan<char> textSpan, EncodeSettings settings)
		{
			...
			var (inputIdsBuffer, mappingPairsBuffer) = TransformersWrapper.TokenizerTextWithOffsets(TokenizerInstance, text, addSpecialTokens: AddSpecialTokens);
		...
		{
			string result = TransformersWrapper.TokenizerDecode(TokenizerInstance, [.. ids.Select(i => (long)i)], skipSpecialTokens: AddSpecialTokens);
